# Network Intrusion Detection - Experiment 2
# XGBoost + SMOTE
# SMOTE creates fake but realistic examples of rare classes
# so the model has more to learn from for R2L and U2R
...
# it creates synthetic examples so all classes end up equal size
# never apply to test data - that would be cheating
...
print(f"experiment 1 was: 0.5599")
print(f"difference: {score - 0.5599:+.4f}")
...
plt.ylabel("Actual")
plt.title(f"Experiment 2 - XGBoost + SMOTE\nMacro F1: {score:.4f}")
plt.tight_layout()
plt.savefig("exp2_confusion_matrix.png", dpi=150)
plt.show()

print("\ndone - saved as exp2_confusion_matrix.png")
